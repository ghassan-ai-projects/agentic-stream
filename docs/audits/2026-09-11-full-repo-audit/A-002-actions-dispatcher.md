...
LOC: 892 · Audit date: 2026-09-11 · Verdict: FIXED
...
- **F1 (HIGH)** fixed: expired leased rows restore the trusted tenant and intent identity loaded from the command ledger before finalization. This lets the reclaim path durably record the unknown outcome and required reconciliation without emitting an incomplete lifecycle event. `TestDispatcherReclaimsExpiredLease` proves the effector is not called, the command becomes `reconciling`, the outbox row becomes `failed`, and both lifecycle notifications retain tenant/intent identity.
- **F2** fixed: the unused `leasedCommand.Now` field and assignment were removed.
- **F3** fixed: `RowsAffected` errors are returned with context instead of being treated as a lost race.
- Verified: `go test -race ./internal/actions` passes.
